...
1. Upload `grill-room.json` at the grill-with-me web app to get a room link
2. Share the link with the team; each member picks a role and joins from
   their own repo checkout
3. After everyone's spec is committed, come back and run the
   `merge-contract` skill
